...
import {
  respondToMatchByFamily,
  type FamilyRespondState,
} from '@/lib/match';

const initial: FamilyRespondState = { ok: false };
...
export function RespondPanel({
  ...
  const [state, action] = useFormState(respondToMatchByFamily, initial);
          ...
          <form action={action} className="flex flex-col gap-2">
            ...
            <label className="flex flex-col gap-1.5 text-sm">
              <span className="font-body text-[0.7rem] font-medium uppercase tracking-[0.08em] text-stone">
                Tell us why - it helps us match better next time
              </span>
              <textarea
                name="note"
                required
                ...
                maxLength={2000}
                rows={3}
                placeholder="Brief reason. Stays internal - the companion does not see your words."
