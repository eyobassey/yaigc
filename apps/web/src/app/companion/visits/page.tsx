import { Calendar } from 'lucide-react';
import { requireCompanion } from '@/lib/auth-helpers';

export const metadata = { title: 'Visits' };

export default async function CompanionVisitsPlaceholder() {
  await requireCompanion('/companion/visits');
  return (
    <div className="max-w-[640px]">
      <header className="mb-6 flex items-center gap-3">
        <Calendar size={22} strokeWidth={1.75} className="text-moss" aria-hidden="true" />
        <h1 className="font-head font-normal text-moss text-[clamp(1.75rem,3vw,2.25rem)] leading-[1.1]">
          Visits
        </h1>
      </header>
      <p className="text-charcoal text-[0.9375rem] leading-[1.55]">
        Coming online shortly. You will see todays visit, your upcoming
        rhythm, and the visits you have already done with their notes
        and any photos you uploaded.
      </p>
    </div>
  );
}
