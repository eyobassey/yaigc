...
- `71a23ce` · **U.3 + U.4 + U.5 + V.1 → V.5** — bundled commit shipping the cover-introduction scheduling hint, candidates-considered audit log, schema for recipient-voice channels, expanded two-visit-review form, and the match-detail surface that lays out which channels reached the recipient. **ADR 0007** (`docs/adr/0007-sdd-addendum-folded.md`) records that both §6 fold-in triggers have fired — addendum content is now part of the SDD master (cover-companion + two-visit calibration → §11.3 + §11.4; cultural-fit refinement → §10.6.3). Cover-is-a-nudge deviation explicitly preserved. Audit log learns channel presence (`channels.{companionCall, familyCall, recipientCall}`) so direct-recipient call rate becomes measurable. Schema delta: `Visit.coverSuggested Boolean default false`; `PostVisitReport.recipientPerspective text nullable`; `Match.twoVisitReviewCompanionCallNotes` / `FamilyCallNotes` / `RecipientCallNotes` / `RecipientCalledAt`
- `d7bd161` · **U.2** — side-by-side primary + cover proposal UX. New `previewCandidate` returns a single companion's enrichment + gate failures (DBS expired / capacity maxed / travel >60 min / archived) so the propose page can warn while still letting the operator proceed. ProposeForm reads `?primary=` / `?cover=` / `?recipientId=` query params, renders two preview cards (photo / borough / rate / load / travel / DBS / shared interests / gate-failure warnings). Candidates page gains a "Lock as primary, pick cover" two-step flow: each card has a new secondary button; when locked, the page banners the primary and remaining cards swap CTA to "Pick as cover"
- `bf2bfd7` · **U.1** — candidate filter + scoring view at `/ops/families/[id]/match-candidates`. Hard filters: DBS clear, capacity not maxed, drive time ≤60 min. Composite score: `interestOverlap × 3 + availabilityFlexibility × 2 − min(drivingMinutes / 5, 20) − currentMatches × 0.5`. Interest overlap is a lowercased word-token intersection with stopwords stripped. Open match count comes from a single `Match.groupBy` query, no N+1. Each card surfaces the score, shared interests by name, availability summary, travel + load penalties. Family-detail page gains a "Find a companion" pill alongside the existing "Propose a match" link
