...
apply form. Stage U (matching surface) is in: ranked candidate filter
+ scoring at `/ops/families/[id]/match-candidates`, side-by-side
primary + cover proposal UX with travel + DBS + gate-failure
warnings, candidates-considered audit log, and cover-introduction
scheduling suggestions. Stage V (recipient voice in the two-visit
review) is in: companion-relayed perspective on every post-visit
report, three new operator capture sections on the review form
...
- [x] SDD Addendum — Three patterns (S + T + U + V) **folded into the SDD master** (ADR 0007, 2026-05-27). Cover-companion fields + two-visit calibration → SDD §11.3 + §11.4; cultural-fit dimensions → SDD §10.6.3. The build deviated from the addendum's literal "cover required at acceptance" wording — cover is a nudge, not a gate (see `memory/project_cover_companion_non_blocking.md`); the SDD master should reflect that. Open question §4.6 (recipient voice in the two-visit review) resolved in Stage V via four channels: companion-relayed perspective on every post-visit report (V.2), companion debrief notes, family debrief notes, optional operator → recipient call (V.3), with the review surface honestly laying out which channels reached the recipient this round (V.4). Cover-introduction scheduling (U.4) uses the addendum's literal "1 in 5" arithmetic — every 5th visit within the first 60 days. Matching surface (U.1–U.3) is the second §6 trigger that fired: ranked candidate filter, side-by-side proposal, candidates-considered audit log
